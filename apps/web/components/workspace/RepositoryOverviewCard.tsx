...
import { Badge, StatBlock, Surface, Tilt } from "@blueprint/ui";
    ...
    <Tilt maxTilt={3}>
      <Surface padding="md" className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-4 border-t border-ink-950/6 pt-4 dark:border-white/8">
          <StatBlock label="Files" value={<CountUp value={fileCount} />} />
          <StatBlock label="Modules" value={<CountUp value={moduleCount} />} />
          <StatBlock label="Imports" value={<CountUp value={importCount} />} />
          <StatBlock
            label="Confidence"
            value={confidencePercent === null ? "—" : <CountUp value={confidencePercent} suffix="%" />}
          />
        </div>
      </Surface>
    </Tilt>
